...
from .serializers import CreateUserSerializer,LoginSerializer,ResetPasswordSerializer,VerifyOTPSerializer,GenerateOTPserializer,ProfileSetupSerializer
...
class UserCreateView(APIView):
    @swagger_auto_schema(
            ...
            responses={
                201: openapi.Response('Register Successful',schema=openapi.Schema(
    ...
    def post(self,request):
        ...
        if serializer.is_valid():
            ...
            response =  Response({'message':'User created successfully','access_token':access_token,'is_new_user':True},status=status.HTTP_201_CREATED)
            response.set_cookie(
                key='refresh_token',
                value=refresh_token,
                httponly=True,
...
class LoginView(APIView):
    ...
    def post(self,request):
        ...
        try:
            ...
            return Response({"message": "User has no profile assigned"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception:
...
class ProfileSetupView(APIView):
    ...
    def post(self,request):
        ...
        profile_serializer = ProfileSetupSerializer(user_profile, data=request.data, partial=True)
